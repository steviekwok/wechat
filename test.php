<?php
//use lib\Fun;
include 'web/conf/config.php';
function __autoload($class) {
    //echo '---------------'.$class.'---------------'.'<br>';
    include  'web/' . str_replace('\\','/',$class) . '.php';
}
class Fun{
     public $a=1;
}
$obj = new Fun; // 用use这里可以不用lib\Fun()，用use这里再定义classFun会出错（已定义类）
echo $obj->a;
//header("Content-Type: text/html; charset=UTF8");
$img = 'http://wechat.steviekwok.top/web/upload/img/79496088.jpg';
...
$api = "https://api-cn.faceplusplus.com/facepp/v3/detect?api_key=aEWCBGl3Prs83F343TsxIbd1B50xLVts&api_secret=XCWVroKcFwUwY0lfzzbVv7P17J_gkDFH&image_url={$img}&return_attributes=gender,age,smiling,ethnicity,beauty,skinstatus";
$ch = curl_init();
//设置我们请求的地址
